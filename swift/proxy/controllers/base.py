...
def get_container_info(env, app, swift_source=None):
    ...
    if not info:
        ...
        #
        # XXX this probably can't pick up a different
        # auto_create_account_prefix since app.auto_create_account_prefix is
        # only defined if it's the proxy server... and this was like that
        # when I got here :p
        #
...
def set_info_cache(app, env, account, container, resp):
    ...
    :returns: the info that was placed into the cache
    """
...
def _prepare_pre_auth_info_request(env, path, swift_source):
    ...
    # Make per-request caching work if possible
    if 'swift.infocache' in env:
        newenv['swift.infocache'] = env['swift.infocache']
