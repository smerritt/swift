...
class ContainerController(object):
    ...
    def account_update(self, req, account, container, broker):
        ...
        """
        account_hosts = req.headers.get('X-Account-Host', '')
        account_devices = req.headers.get('X-Account-Device', '')
        account_partition = req.headers.get('X-Account-Partition', '')

        updates = [upd for upd in
                   zip((h.strip() for h in account_hosts.split(',')),
                       (c.strip() for c in account_devices.split(',')))
                   if all(upd) and account_partition]
