...
class TestObjectController(unittest.TestCase):
    ...
    def test_policy_IO(self):
        ...
        def check_file(ring_num, cont, devs, check_val):
            partition, nodes = prosrv.get_object_ring(ring_num).get_nodes(
                'a', cont, 'o')
            for dev in devs:
                file = diskfile.DiskFile(_testdir, dev, partition, 'a', cont,
                                         'o', FakeLogger(), keep_data_fp=True)
                if file.data_file is None and check_val == False:
                    return
        ...
        #check ring 0: put file on c, read it back, check location on disk
        sock = connect_tcp(('localhost', prolis.getsockname()[1]))
        fd = sock.makefile()
        obj = 'test_obejct'
        path = '/v1/a/c/o'
        ...
        #check ring 1: put file on c1, read it back, check location on disk
        sock = connect_tcp(('localhost', prolis.getsockname()[1]))
        fd = sock.makefile()
        path = '/v1/a/c1/o'
        fd.write('PUT %s HTTP/1.1\r\n'
        ...
        #check ring 2: put file on c2, read it back, check location on disk
        sock = connect_tcp(('localhost', prolis.getsockname()[1]))
        fd = sock.makefile()
        path = '/v1/a/c2/o'
        fd.write('PUT %s HTTP/1.1\r\n'
